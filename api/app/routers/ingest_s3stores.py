...
from ..models.ingest_s3store import IngestS3Store, IngestS3StoreCreate, IngestS3StorePublic, IngestS3StoreUpdate
...
@router.get("/", response_model=list[IngestS3StorePublic], summary=f"Get a list of {entity_name}")
def read_list(
        *,
...
@router.get("/{id}", response_model=IngestS3StorePublic, summary=f"Get one {entity_name}")
...
@router.post("/", response_model=IngestS3StorePublic, summary=f"Create one {entity_name}")
def create(*, session: Session = Depends(get_session), payload: IngestS3StoreCreate, user=Depends(get_current_user)):
...
@router.patch("/{id}", response_model=IngestS3StorePublic, summary=f"Update one {entity_name}")
