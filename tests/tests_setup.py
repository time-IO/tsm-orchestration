...
if SMS_ACCESS_TYPE == "api":
    SMS_API_URL = getenv("SMS_URL") + "backend/api/v1/"
...
if CV_ACCESS_TYPE == "api":
    CV_API_URL = getenv("CV_URL") + "api/v1"
...
OBS_DB = mk_postgres_dsn(
    ...
    getenv("CREATEDB_POSTGRES_PORT", 5432, cast_to=int),  # no env var !!
...
FE_DB = mk_postgres_dsn(
    ...
    getenv("FRONTEND_POSTGRES_PORT", 5432, cast_to=int),  # no env var !!
