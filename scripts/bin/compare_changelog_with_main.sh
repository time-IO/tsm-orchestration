...
if git diff main $BRANCH -- CHANGELOG.md | grep -q .; then
    ...
    if [ "$LAST_CHANGE_MAIN" -gt "$LAST_CHANGE_BRANCH" ]; then
        echo -e "${WARNING}Oh no!${BOLD}\nCHANGELOG.md on '$BRANCH' is behind changelog on 'main'.${ENDC}"
        exit 8 # let the pipeline pass with a warning
    else
        echo -e "${OK}Looking good!${BOLD}\nCHANGELOG.md is up to date.${ENDC}"
        exit 0
    fi
else
    echo -e "${WARNING}Oh no!${BOLD}\nNo changelog entry has been made so far. Don't forget to add one!${ENDC}"
    exit 8 # let the pipeline pass with a warning
fi
