               ...
               'when_dynamic', ARRAY_AGG(
                   DISTINCT CASE
                       WHEN cdl.end_date IS NULL THEN
                           TO_CHAR(cdl.begin_date, 'YYYY-MM-DD HH24:MI:SS TZ')
                       ELSE
                           TO_CHAR(cdl.begin_date, 'YYYY-MM-DD HH24:MI:SS TZ')  || '/' || TO_CHAR(cdl.end_date, 'YYYY-MM-DD HH24:MI:SS TZ')
                   END
                   ),
               'when_stationary', ARRAY_AGG(
                   DISTINCT CASE
                       WHEN csl.end_date IS NULL THEN
                           TO_CHAR(csl.begin_date, 'YYYY-MM-DD HH24:MI:SS TZ')
                       ELSE
                           TO_CHAR(csl.begin_date, 'YYYY-MM-DD HH24:MI:SS TZ')  || '/' || TO_CHAR(csl.end_date, 'YYYY-MM-DD HH24:MI:SS TZ')
                   END
                   ),
               'contacts', ARRAY_AGG(
...
GROUP BY
    c.id, c.description, c.label, c.persistent_identifier, c.status, c.is_public,
...
HAVING
    ...
    AND dsl.datasource_id = %(tsm_schema)s AND c.is_public
ORDER BY c.id ASC;
...
JOIN public.sms_configuration_contact_role ccr  ON dma.configuration_id = ccr.configuration_id
JOIN public.sms_contact co                      ON ccr.contact_id = co.id
JOIN public.sms_datastream_link dsl             ON dma.id = dsl.device_mount_action_id
GROUP BY
    d.id, d.short_name, d.description, d.persistent_identifier, d.device_type_name,
    d.manufacturer_name, d.model, d.serial_number, d.is_public, dsl.datasource_id
HAVING dsl.datasource_id = %(tsm_schema)s AND d.is_public
ORDER BY d.id ASC;
...
JOIN public.sms_device_mount_action dma ON c.id = dma.configuration_id
JOIN public.sms_datastream_link dsl ON dma.id = dsl.device_mount_action_id
WHERE dsl.datasource_id = %(tsm_schema)s AND c.is_public
ORDER BY csl.id;
...
SELECT
    loc.max_csl_id AS "LOCATION_ID",
    cl.csl_id AS "HIST_LOCATION_ID"
-- join locations on newest location on configuration id
FROM config_locations cl JOIN locations loc ON cl.c_id = loc.c_id
...
SELECT DISTINCT
    ...
    "TIME"
FROM cte
JOIN public.sms_device_mount_action dma on cte."THING_ID" = dma.configuration_id
JOIN public.sms_datastream_link dsl on dma.id = dsl.device_mount_action_id
WHERE row_num > 1 AND dsl.datasource_id = %(tsm_schema)s;
...
JOIN observation o ON o.datastream_id = dsl.datastream_id
JOIN public.sms_device_mount_action dma ON dma.id = dsl.device_mount_action_id
JOIN ts_action_type ts_type ON ts_type.result_time = o.result_time
LEFT JOIN location_property_order lpo ON lpo.device_property_id = dsl.device_property_id
JOIN ts_coordinates crd ON crd.result_time = o.result_time;
-- filter on device and configuration already happening in ts_action_type

DROP VIEW IF EXISTS "datastreams_observed_areas" CASCADE;
create or replace view "datastreams_observed_areas" as
select
    dsl.device_property_id as ds_id,
    public.st_convexhull(
        public.ST_collect(
            public.ST_Makepoint(
                (ft."FEATURE"->'geometry'->'coordinates')[0][0]::double precision,
                (ft."FEATURE"->'geometry'->'coordinates')[0][1]::double precision,
                (ft."FEATURE"->'geometry'->'coordinates')[0][2]::double precision
            )
        )
    ) AS "obs_area"
from public.sms_datastream_link dsl
join "OBSERVATIONS" obs ON dsl.device_property_id = obs."DATASTREAM_ID"
join "FEATURES" ft ON obs."FEATURE_ID" = ft."ID"
group by dsl.device_property_id, dsl.datasource_id
having dsl.datasource_id = %(tsm_schema)s;
...
CREATE OR REPLACE VIEW "DATASTREAMS" AS SELECT DISTINCT
    ...
    -- envelope of all foi for datastream
    dsoa.obs_area AS "OBSERVED_AREA",
    ...
    LEFT JOIN datastreams_observed_areas dsoa ON dsl.device_property_id = dsoa.ds_id
    WHERE dsl.datasource_id = %(tsm_schema)s AND c.is_public AND d.is_public;
...
WHERE (cdl.configuration_id IS NOT NULL OR csl.configuration_id IS NOT NULL) AND c.is_public AND d.is_public
AND dp.property_uri <>''
