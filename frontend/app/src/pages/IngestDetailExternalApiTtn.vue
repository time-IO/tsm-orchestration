          ...
          <div class="row q-col-gutter-md">
            <div class="col-md-6">
              <q-list>
                ...
                <q-item>
                  <q-item-section>
                    <q-item-label>Endpoint-URI</q-item-label>
                    <q-item-label caption>{{ item.endpoint_uri }}</q-item-label>
                  </q-item-section>
                </q-item>
...
const item = ref<IngestExternalApiTheThingsNetworkPublic | null>(null);
...
const editRoute = computed(() => {
  if (item.value?.id) {
    return `/ingest/external-api/ttn/${item.value.id}/edit`;
  }
  return '';
});
