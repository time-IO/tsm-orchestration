...
async function save() {
  try {
    const data: CsvParserCreate = {
      ...
      headlines_to_exclude: formData.value.headlines_to_exclude,
      footlines_to_exclude: formData.value.footlines_to_exclude,
    ...
    await router.push(`/ingest/parser/csv/${result.id}`);
  } catch (error) {
