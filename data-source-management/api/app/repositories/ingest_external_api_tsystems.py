...
from models import IngestExternalApiTSystems, IngestExternalApi, Ingest
...
class IngestExternalApiTSystemsRepository:
    def __init__(self, session: Session):
        self.model = IngestExternalApiTSystems
    ...
    def create(
        ...
        try:
            ...
            data_ingest_external_api_tsystems = {"ingest_id": ingest.id}
            ingest_external_api_tsystems = IngestExternalApiTSystems.model_validate(
                payload, update=data_ingest_external_api_tsystems
            )

            self.session.add(ingest_external_api_tsystems)

            self.session.commit()

            return ingest_external_api_tsystems
    ...
    @staticmethod
    def to_flat(tsystems: IngestExternalApiTSystems) -> IngestExternalApiTSystemsRead:
        ext = tsystems.external_api
        ing = ext.ingest
        permission_group = ing.permission_group

        return IngestExternalApiTSystemsRead(
            # Ingest
            id=ing.id,
            uuid=ing.uuid,
            ...
            # TSystems
            group=tsystems.group,
            station_id=tsystems.station_id,
            tsystems_username=tsystems.tsystems_username,
            tsystems_password=tsystems.tsystems_password,
            # Permission Group
            permission_group={
