...
from models import IngestExternalApiDwd, IngestExternalApi, Ingest
...
class IngestExternalApiDwdRepository:
    def __init__(self, session: Session):
        self.model = IngestExternalApiDwd
    ...
    def create(
        ...
        try:
            ...
            data_ingest_external_api_dwd = {"ingest_id": ingest.id}
            ingest_external_api_dwd = IngestExternalApiDwd.model_validate(
                payload, update=data_ingest_external_api_dwd
            )

            self.session.add(ingest_external_api_dwd)

            self.session.commit()

            return ingest_external_api_dwd
    ...
    @staticmethod
    def to_flat(dwd: IngestExternalApiDwd) -> IngestExternalApiDwdRead:
        ext = dwd.external_api
        ing = ext.ingest
        permission_group = ing.permission_group

        return IngestExternalApiDwdRead(
            # Ingest
            id=ing.id,
            uuid=ing.uuid,
            ...
            # Dwd
            station_id=dwd.station_id,
            period_in_minutes=dwd.period_in_minutes,
            # Permission Group
            permission_group={
