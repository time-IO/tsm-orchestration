...
from models import IngestExternalApiTheThingsNetwork, IngestExternalApi, Ingest
...
class IngestExternalApiTheThingsNetworkRepository:
    def __init__(self, session: Session):
        self.model = IngestExternalApiTheThingsNetwork
    ...
    def create(
        ...
        try:
            ...
            data_ingest_external_api = {
                "api_type": ApiType.THE_THINGS_NETWORK,
                "ingest_id": ingest.id,
            }
            ...
            data_ingest_external_api_ttn = {"ingest_id": ingest.id}
            ingest_external_api_ttn = IngestExternalApiTheThingsNetwork.model_validate(
                payload, update=data_ingest_external_api_ttn
            )

            self.session.add(ingest_external_api_ttn)

            self.session.commit()

            return ingest_external_api_ttn
    ...
    @staticmethod
    def to_flat(
        ttn: IngestExternalApiTheThingsNetwork,
    ) -> IngestExternalApiTheThingsNetworkRead:
        ext = ttn.external_api
        ing = ext.ingest
        permission_group = ing.permission_group

        return IngestExternalApiTheThingsNetworkRead(
            # Ingest
            id=ing.id,
            uuid=ing.uuid,
