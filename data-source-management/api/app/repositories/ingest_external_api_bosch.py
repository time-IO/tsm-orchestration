...
from models import IngestExternalApiBosch, IngestExternalApi, Ingest
...
class IngestExternalApiBoschRepository:
    def __init__(self, session: Session):
        self.model = IngestExternalApiBosch
    ...
    def create(
        ...
        try:
            ...
            data_ingest_external_api_bosch = {"ingest_id": ingest.id}
            ingest_external_api_bosch = IngestExternalApiBosch.model_validate(
                payload, update=data_ingest_external_api_bosch
            )

            self.session.add(ingest_external_api_bosch)

            self.session.commit()

            return ingest_external_api_bosch
    ...
    @staticmethod
    def to_flat(bosch: IngestExternalApiBosch) -> IngestExternalApiBoschRead:
        ext = bosch.external_api
        ing = ext.ingest
        permission_group = ing.permission_group

        return IngestExternalApiBoschRead(
            # Ingest
            id=ing.id,
            uuid=ing.uuid,
            ...
            # Bosch
            endpoint=bosch.endpoint,
            sensor_id=bosch.sensor_id,
            bosch_username=bosch.bosch_username,
            bosch_password=bosch.bosch_password,
            period_in_minutes=bosch.period_in_minutes,
            # Permission Group
            permission_group={
