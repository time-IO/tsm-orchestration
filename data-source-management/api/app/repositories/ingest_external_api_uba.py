...
from models import IngestExternalApiUba, IngestExternalApi, Ingest
...
class IngestExternalApiUbaRepository:
    def __init__(self, session: Session):
        self.model = IngestExternalApiUba
    ...
    def create(
        ...
        try:
            ...
            data_ingest_external_api_uba = {"ingest_id": ingest.id}
            ingest_external_api_uba = IngestExternalApiUba.model_validate(
                payload, update=data_ingest_external_api_uba
            )

            self.session.add(ingest_external_api_uba)

            self.session.commit()

            return ingest_external_api_uba
    ...
    @staticmethod
    def to_flat(uba: IngestExternalApiUba) -> IngestExternalApiUbaRead:
        ext = uba.external_api
        ing = ext.ingest
        permission_group = ing.permission_group

        return IngestExternalApiUbaRead(
            # Ingest
            id=ing.id,
            uuid=ing.uuid,
