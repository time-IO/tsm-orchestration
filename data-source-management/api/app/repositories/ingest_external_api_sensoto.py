...
from models import IngestExternalApiSensoto, IngestExternalApi, Ingest
...
class IngestExternalApiSensotoRepository:
    def __init__(self, session: Session):
        self.model = IngestExternalApiSensoto
    ...
    def create(
        ...
        try:
            ...
            data_ingest_external_api_sens = {"ingest_id": ingest.id}
            ingest_external_api_sens = IngestExternalApiSensoto.model_validate(
                payload, update=data_ingest_external_api_sens
            )

            self.session.add(ingest_external_api_sens)

            self.session.commit()

            return ingest_external_api_sens
    ...
    @staticmethod
    def to_flat(
        sens: IngestExternalApiSensoto,
    ) -> IngestExternalApiSensotoRead:
        ext = sens.external_api
        ing = ext.ingest
        permission_group = ing.permission_group

        return IngestExternalApiSensotoRead(
            # Ingest
            id=ing.id,
            uuid=ing.uuid,
