"""
Integration tests for the ingest/external-api/tsystems router.
...
BASE_PATH = "/ingest/external-api/tsystems"
...
def _tsystems_payload(base_data, **overrides):
    payload = {
        "name": "Integration Test TSystems",
        "permission_group_id": base_data["permission_group_id"],
        "sync_enabled": True,
        "group": "group-1",
        "station_id": "station-1",
        "tsystems_username": "user",
        "tsystems_password": "secret",
    }
...
def test_create_and_read(client, base_data):
    payload = _tsystems_payload(base_data)
    response = client.post(f"{BASE_PATH}/", json=payload)
    assert response.status_code == 200
    created = response.json()
    assert created["name"] == "Integration Test TSystems"
    assert created["station_id"] == "station-1"
    assert created["api_type"] == "tsystems"
...
def test_create_and_update(client, base_data):
    payload = _tsystems_payload(base_data, name="TSystems To Update")
    response = client.post(f"{BASE_PATH}/", json=payload)
    assert response.status_code == 200
...
def test_create_and_delete(client, base_data):
    payload = _tsystems_payload(base_data, name="TSystems To Delete")
    response = client.post(f"{BASE_PATH}/", json=payload)
    assert response.status_code == 200
    ingest_id = response.json()["id"]
...
def test_read_list(client, base_data):
    for name in ["TSystems List A", "TSystems List B"]:
        client.post(f"{BASE_PATH}/", json=_tsystems_payload(base_data, name=name))
...
def test_create_unauthenticated(client_no_auth, base_data):
    response = client_no_auth.post(f"{BASE_PATH}/", json=_tsystems_payload(base_data))
    assert response.status_code == 401


def test_read_list_wrong_group_returns_empty(client, client_other_group, base_data):
    client.post(f"{BASE_PATH}/", json=_tsystems_payload(base_data))
...
def test_read_one_wrong_group_returns_404(client, base_data, other_group_data):
    created_response = client.post(f"{BASE_PATH}/", json=_tsystems_payload(base_data))
