"""
Integration tests for the ingest/external-api/the-things-network router.
...
def _ttn_payload(base_data, **overrides):
    payload = {
        "name": "Integration Test TTN",
        "permission_group_id": base_data["permission_group_id"],
        "sync_enabled": True,
        "sync_interval_in_minutes": 15,
        "api_key": "secret-key",
        "endpoint_uri": "https://example.com/ttn",
    }
...
def test_create_and_read(client, base_data):
    payload = _ttn_payload(base_data)
    response = client.post(f"{BASE_PATH}/", json=payload)
    assert response.status_code == 200
    created = response.json()
    assert created["name"] == "Integration Test TTN"
    assert created["endpoint_uri"] == "https://example.com/ttn"
    assert created["api_type"] == "ttn"
...
def test_create_and_update(client, base_data):
    payload = _ttn_payload(base_data, name="TTN To Update")
    response = client.post(f"{BASE_PATH}/", json=payload)
    assert response.status_code == 200
    ingest_id = response.json()["id"]

    response = client.patch(
        f"{BASE_PATH}/{ingest_id}", json={"endpoint_uri": "https://example.com/ttn-2"}
    )
    assert response.status_code == 200
    assert response.json()["endpoint_uri"] == "https://example.com/ttn-2"


def test_create_and_delete(client, base_data):
    payload = _ttn_payload(base_data, name="TTN To Delete")
    response = client.post(f"{BASE_PATH}/", json=payload)
    assert response.status_code == 200
    ingest_id = response.json()["id"]
...
def test_read_list(client, base_data):
    for name in ["TTN List A", "TTN List B"]:
        client.post(f"{BASE_PATH}/", json=_ttn_payload(base_data, name=name))
...
def test_create_unauthenticated(client_no_auth, base_data):
    response = client_no_auth.post(f"{BASE_PATH}/", json=_ttn_payload(base_data))
    assert response.status_code == 401


def test_read_list_wrong_group_returns_empty(client, client_other_group, base_data):
    client.post(f"{BASE_PATH}/", json=_ttn_payload(base_data))
...
def test_read_one_wrong_group_returns_404(client, base_data, other_group_data):
    created_response = client.post(f"{BASE_PATH}/", json=_ttn_payload(base_data))
