"""
Integration tests for the ingest/external-sftp router.
...
def test_create_and_read(client, base_data):
    ...
    assert created["name"] == "Integration Test Sftp"
    assert created["uri"] == "sftp://example.com"
    assert created["ssh_public_key"]  # generated server-side, just check it's present
...
def test_create_and_update(client, base_data):
    payload = _sftp_payload(base_data, name="Sftp To Update")
    response = client.post(f"{BASE_PATH}/", json=payload)
    assert response.status_code == 200
    ingest_id = response.json()["id"]

    response = client.patch(f"{BASE_PATH}/{ingest_id}", json={"path": "/new-data"})
    assert response.status_code == 200
    assert response.json()["path"] == "/new-data"


def test_create_and_delete(client, base_data):
    payload = _sftp_payload(base_data, name="Sftp To Delete")
    response = client.post(f"{BASE_PATH}/", json=payload)
    assert response.status_code == 200
    ingest_id = response.json()["id"]
...
def test_read_list(client, base_data):
    for name in ["Sftp List A", "Sftp List B"]:
