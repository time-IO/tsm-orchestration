"""
Tests for the ingest/external-api/sensoto router (full CRUD).
...
from dependencies import get_repo_ingest_external_api_sensoto

ROUTER_MODULE = "routers.ingest_external_api_sensoto"
BASE_PATH = "/ingest/external-api/sensoto"


def _sensoto_dict(make_ingest_dict, **overrides):
    """Builds a dict for IngestExternalApiSensotoRead: IngestRead fields
    (via make_ingest_dict) + IngestExternalApiRead fields (api_type,
    sync_enabled, sync_interval_in_minutes) + sensoto-specific fields."""
    defaults = {
        "api_type": "sensoto",
        "sync_enabled": True,
...
def test_read_one_found(client, override_repo, make_ingest_dict):
    repo = override_repo(get_repo_ingest_external_api_sensoto)
    repo.find_one.return_value = object()  # content irrelevant, to_flat is mocked
    repo.to_flat.return_value = _sensoto_dict(make_ingest_dict)

    response = client.get(f"{BASE_PATH}/1")

    assert response.status_code == 200
    assert response.json()["network"] == "network-1"
