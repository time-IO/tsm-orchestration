"""
Tests for the ingest/external-api/neutron_monitor router (full CRUD).
...
from dependencies import get_repo_ingest_external_api_neutron_monitor

ROUTER_MODULE = "routers.ingest_external_api_neutron_monitor"
BASE_PATH = "/ingest/external-api/neutron-monitor"


def _neutron_monitor_dict(make_ingest_dict, **overrides):
    """Builds a dict for IngestExternalApineutron_monitorRead: IngestRead fields
    (via make_ingest_dict) + IngestExternalApiRead fields (api_type,
    sync_enabled, sync_interval_in_minutes) + neutron_monitor-specific fields.
    Merge first, then call - otherwise an override collides with a
    same-named default keyword arg (e.g. station_id)."""
    defaults = {
        "api_type": "neutron_monitor",
        "sync_enabled": True,
        "sync_interval_in_minutes": None,
        "station_id": 1,
        "time_resolution_in_minutes": 60,
        "station": {"id": 1, "name": "NeutronMonitorStation"},
    }
...
def test_read_one_found(client, override_repo, make_ingest_dict):
    repo = override_repo(get_repo_ingest_external_api_neutron_monitor)
    repo.find_one.return_value = object()  # content irrelevant, to_flat is mocked
    repo.to_flat.return_value = _neutron_monitor_dict(make_ingest_dict)
