...
from dependencies import get_repo_ingest_external_api_tsystems

ROUTER_MODULE = "routers.ingest_external_api_tsystems"
BASE_PATH = "/ingest/external-api/tsystems"


def _tsystems_dict(make_ingest_dict, **overrides):
    """Builds a dict for IngestExternalApitsystemsRead: IngestRead fields
    ...
    defaults = {
        ...
        "tsystems_username": "user1",
        "tsystems_password": "password",
    }
...
def test_read_one_found(client, override_repo, make_ingest_dict):
    repo = override_repo(get_repo_ingest_external_api_tsystems)
    repo.find_one.return_value = object()  # content irrelevant, to_flat is mocked
    repo.to_flat.return_value = _tsystems_dict(make_ingest_dict)
...
def test_update(client, override_repo, make_ingest_dict, mock_publish_frontend_update):
    ...
    repo.to_flat.return_value = _tsystems_dict(make_ingest_dict, station_id="station-2")
    mock_publish = mock_publish_frontend_update(ROUTER_MODULE)
