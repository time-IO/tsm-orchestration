...
BEGIN
    FOREACH table_name IN ARRAY table_names LOOP
        ...
        IF EXISTS (SELECT 1 FROM information_schema.foreign_tables WHERE foreign_table_name = ft_name AND foreign_table_schema = 'public') THEN
            EXECUTE format('CREATE MATERIALIZED VIEW public.%s AS SELECT * FROM public.%s;', materialized_view_name, ft_name);
            EXECUTE format('CREATE UNIQUE INDEX %s_pkey ON public.%s (id);', materialized_view_name, materialized_view_name);
